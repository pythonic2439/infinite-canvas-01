...
return (edited.data || []).map((item) => item.b64_json ? \`data:image/png;base64,\${item.b64_json}\` : item.url);`,
        ...
        {
            label: i18n.t("modelPlugin.templates.jimeng"),
            script: `// ${i18n.t("modelPlugin.templates.imageJimeng")}
// ${i18n.t("modelPlugin.templates.availableImageJimeng")}
// POST /v1/images/generations — ${i18n.t("modelPlugin.templates.jimengUrlNote")}
...
// ${i18n.t("modelPlugin.templates.jimengRatioFromSize")}
const match = \`\${params.size || ""}\`.match(/(\\d+)x(\\d+)/);
const width = match ? Number(match[1]) : 0;
const height = match ? Number(match[2]) : 0;
const ratio = RATIOS.includes(params.ratio || "") ? params.ratio : closestRatio(width, height) || "1:1";
const body = new FormData();
body.set("model", model);
body.set("prompt", prompt);
body.set("ratio", ratio);
body.set("response_format", "b64_json");
// ${i18n.t("modelPlugin.templates.jimengMultiRef")}
for (const dataUrl of images) {
  body.append("image", await (await fetch(dataUrl)).blob(), "ref.png");
}
const data = await request({
  method: "post",
...
if (data && data.code !== undefined && data.code !== 0) throw new Error(data.message || "jimeng API error");
return (data.data || []).map((item) => item.b64_json ? \`data:image/png;base64,\${item.b64_json}\` : item.url);`,
        },
        {
...
return await poll(
  ...
  { intervalMs: 2500, timeoutMs: 300000 },
        ...
        {
            label: i18n.t("modelPlugin.templates.jimeng"),
            script: `// ${i18n.t("modelPlugin.templates.videoJimeng")}
// ${i18n.t("modelPlugin.templates.availableVideoJimeng")}
// ${i18n.t("modelPlugin.templates.jimengUrlNote")}
const RATIOS = { "1280x720": "16:9", "720x1280": "9:16" };
const ratio = /^\\d+:\\d+$/.test(params.ratio || "") ? params.ratio : (RATIOS[params.size] || "16:9");
const body = new FormData();
body.set("model", model);
body.set("prompt", prompt);
body.set("ratio", ratio);
body.set("resolution", params.resolution || "720p");
// ${i18n.t("modelPlugin.templates.jimengDuration")}
const duration = Number(params.seconds) >= 10 ? 10 : 5;
body.set("duration", duration);
for (const dataUrl of images) {
  body.append("file_paths", await (await fetch(dataUrl)).blob(), "ref.png");
}
const data = await request({
  method: "post",
...
if (!url) throw new Error(${JSON.stringify(i18n.t("modelPlugin.templates.jimengNoVideoUrl"))});
return { url };`,
        },
        {
            label: i18n.t("modelPlugin.templates.gemini"),
