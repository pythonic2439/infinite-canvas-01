...
+ [新增] 渠道新增 dola 协议：内置 DOLA 预设渠道（Base URL 经本站 /dola-proxy 反代），支持 dola-image / dola-video 异步任务，参考图自动转公网链接提交。
+ [新增] 图片节点工具栏新增「眼部网格」防人脸拦截：本地检测人脸并对眼部/整脸叠加干扰（MediaPipe 离线模型），渠道开启后仅对视频参考图自动处理，图片生图不拦截。
+ [新增] 生成结果为临时外链时统一下载并本地保存展示，下载失败自动回退原链接；dola-image 渠道比例选项收敛为 1:1 / 16:9 / 9:16 三档。
+ [修复] dola 参考图过大触发上传上限：仅当原图超过 20MB 上限时才逐步降采样转 JPEG，未超限原样上传不损画质。
+ [修复] 画布参考图转公网链接时报"file too large"：blob: 对象 URL 未经转换直接上传产生空文件，现统一先转 data URL；上传接口对空文件与超限文件分开报错。
+ [新增] 渠道设置新增"失败自动重试"次数：dola 图片/视频任务失败后自动重新提交（预设 dola 渠道默认 1 次），每次重试会重新消耗生成额度。
+ [优化] 同源请求网络闪断不再误报"跨域策略拦截"，改为提示网络连接中断；闪断多为移动网络/NAT 掐断长连接所致，重试即可恢复。
